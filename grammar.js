...
module.exports = grammar({
  ...
  rules: {
    ...
    declaration: ($) =>
      choice(
        field("local_declaration", $.local_variable_declaration),
        field("local_declaration", $.local_function_declaration),
        field("local_declaration", $.local_class_declaration),
        field("global_declaration", $.global_variable_declaration),
        field("global_declaration", $.global_function_declaration),
        field("global_declaration", $.global_class_declaration),
      ),
    ...
    local_function_declaration: ($) => seq("local", $.function_declaration),

    local_class_declaration: ($) => seq("local", $.class_declaration),

    global_function_declaration: ($) => seq("global", $.function_declaration),

    global_class_declaration: ($) => seq("global", $.class_declaration),
    ...
    loop_statement: ($) =>
      seq("loop", optional("::"), field("body", $.statement)),
    ...
    _for_init: ($) =>
      choice(
        choice($.local_variable_declaration, $.global_variable_declaration),
        seq($.assignment, ";"),
      ),
    ...
    default_case: ($) =>
      seq("default", ":", optional(field("body", $.case_body))),
    ...
    number: (_) => {
      ...
      const decliteral = choice(
        seq(".", repeat1(ddig), optional(exponent("e"))),
        seq(
      ...
      const hexliteral = seq(
        choice("0x", "0X"),
        choice(
          seq(".", repeat1(hdig), optional(exponent("p"))),
          seq(
            hdig,
    ...
    function_call: ($) => seq($.final_call, optional($.call_check_char)),
    ...
    _block: ($) =>
      prec(
        ...
        choice(
          ...
          seq(repeat(choice($.declaration, $.statement)), $.return_statement),
        ),
      ),

    class_definition: ($) =>
      prec.right(seq("class", optional($._inherit), optional($.class_body))),
    ...
    _class_value: ($) =>
      choice(field("method", $.method), field("metafield", $.metafield)),
